...
import (
	...
	"github.com/google/go-eventlog/extract/gmes"
...
func TestVerifyEventLogs(t *testing.T) {
	tpmQuote := testTPMQuote(gmesEventLogData, nil, gmesPCRBanks, &attestpb.TpmAttestationEndorsement{
		Endorsement: &attestpb.TpmAttestationEndorsement_TitanEndorsement_{
			TitanEndorsement: &attestpb.TpmAttestationEndorsement_TitanEndorsement{
	...
	expectedState := &attestpb.HostACOSState{
...
func TestVerifyEventLogsWithCEL(t *testing.T) {
	...
	for _, pcr := range []*tpmpb.PCRs{gmesPCRs} {
	...
	tpmQuote := testTPMQuote(tcgELData, celLaunchEventLogData, []*tpmpb.PCRs{pcrs}, &attestpb.TpmAttestationEndorsement{
		Endorsement: &attestpb.TpmAttestationEndorsement_TitanEndorsement_{
			TitanEndorsement: &attestpb.TpmAttestationEndorsement_TitanEndorsement{
				DiceCertChain: titanDiceChainDataDev,
				EkCert:        ekcDataDev,
...
func testTPMQuote(tpmEventLog []byte, launcherEventLog []byte, pcrBanks []*tpmpb.PCRs, endorsement *attestpb.TpmAttestationEndorsement) *attestpb.TpmQuote {
	var quotes []*attestpb.TpmQuote_SignedQuote
	for _, pcrBank := range pcrBanks {
		quote := &attestpb.TpmQuote_SignedQuote{
...
func TestParseCPUPIIDErrors(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			pcrBank := convertToPCRBank(t, &tpmpb.PCRs{
				Hash: tpmpb.HashAlgo_SHA256,
				Pcrs: map[uint32][]byte{
...
func convertToPCRBank(t *testing.T, pcrs *tpmpb.PCRs) register.PCRBank {
	t.Helper()
...
func testHostAttestation(t *testing.T) *attestpb.HostAttestation {
	t.Helper()
...
func TestValidateTitanEndorsement(t *testing.T) {
	endorsement := &attestpb.TpmAttestationEndorsement_TitanEndorsement{
		DiceCertChain: titanDiceChainDataDev,
		EkCert:        ekcDataDev,
	}
...
func TestTitanEndorsementProd(t *testing.T) {
	attestation := &attestpb.HostAttestation{}
...
func testValidGMESLog(t *testing.T) (rawLog []byte, pcrs *tpmpb.PCRs, expectedState *spb.GMESState) {
...
func testEventLog(t *testing.T, events []tcg.Event) ([]byte, *tpmpb.PCRs) {
	t.Helper()
