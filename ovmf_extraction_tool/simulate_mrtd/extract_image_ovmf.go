...
func main() {
	...
	// Sanitize input to prevent directory traversal by using only the base name.
	// This enforces that the file must be in the current directory.
	...
	// Call the cleanTdx helper function to process the measurement
	mrtdHex := cleanTdx(fw)

	// Print the result directly to the command line
	fmt.Printf("MRTD: %s\n", mrtdHex)
}
