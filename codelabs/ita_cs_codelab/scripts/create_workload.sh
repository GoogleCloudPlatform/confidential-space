#!/bin/bash
#
# Creates GCS data access workload and publishes the workload docker image to artifact registry.
...
cp "${PARENT_DIR}"/src/sample_ita_workload.py "${PARENT_DIR}"/src/workload.py

sed -i'' "s/PRIMUS_INPUT_STORAGE_BUCKET_VALUE/"${PRIMUS_INPUT_STORAGE_BUCKET}"/" "${PARENT_DIR}"/src/workload.py
sed -i'' "s/PRIMUS_PROJECT_ID_VALUE/"${PRIMUS_PROJECT_ID}"/" "${PARENT_DIR}"/src/workload.py
sed -i'' "s/PRIMUS_WORKLOAD_IDENTITY_POOL_VALUE/"${PRIMUS_WORKLOAD_IDENTITY_POOL}"/" "${PARENT_DIR}"/src/workload.py
sed -i'' "s/PRIMUS_WIP_PROVIDER_VALUE/"${PRIMUS_WIP_PROVIDER}"/" "${PARENT_DIR}"/src/workload.py
sed -i'' "s/PRIMUS_PROJECT_NUMBER_VALUE/"${PRIMUS_PROJECT_NUMBER}"/" "${PARENT_DIR}"/src/workload.py
sed -i'' "s/SECUNDUS_PROJECT_ID_VALUE/"${SECUNDUS_PROJECT_ID}"/" "${PARENT_DIR}"/src/workload.py
sed -i'' "s/SECUNDUS_PROJECT_NUMBER_VALUE/"${SECUNDUS_PROJECT_NUMBER}"/" "${PARENT_DIR}"/src/workload.py
sed -i'' "s/SECUNDUS_RESULT_STORAGE_BUCKET_VALUE/"${SECUNDUS_RESULT_STORAGE_BUCKET}"/" "${PARENT_DIR}"/src/workload.py

echo "Building the workload docker image ..."
cd "${PARENT_DIR}"/src

if ! docker build . -t "${IMAGE_REFERENCE}"; then
  err "Failed to build workload docker image "${IMAGE_REFERENCE}"."
  exit 1
fi

echo "Pushing workload docker image to artifact registry "${PRIMUS_ARTIFACT_REPOSITORY}" ..."
if ! docker push "${IMAGE_REFERENCE}"; then
  err "Failed to publish workload docker image "${IMAGE_REFERENCE}" to "${PRIMUS_ARTIFACT_REPOSITORY}"."
  exit 1
fi
