...
delete_service_account() {
  gcloud iam service-accounts list --location=${2} | grep ${1}
  if [[ $? -eq 0 ]]; then
    echo "Deleting service-account ${1}..."
    gcloud iam service-accounts delete ${1} --quite
    if [[ $? -eq 0 ]]; then
...
delete_workload_identity_pool() {
  ...
  if [[ $? -eq 0 ]]; then
    ...
    gcloud iam workload-identity-pools delete ${1} --location==${2} --quite
    if [[ $? -eq 0 ]]; then
...
delete_artifact_repository() {
  ...
  if [[ $? -eq 0 ]]; then
    ...
    gcloud artifacts repositories delete ${1} --location=${2} --async  --quite
    if [[ $? -eq 0 ]]; then
      echo "Artifact repository ${SECUNDUS_ARTIFACT_REPOSITORY} is deleted successfully."
    else
