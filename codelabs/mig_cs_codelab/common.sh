...
#######################################
# Grants roles/artifactregistry.writer to service-account.
# Globals:
#   None
# Arguments:
#   Workload service account
#   Project ID
#######################################
grant_artifact_registry_writer_role_to_service_account() {
  local workload_service_account=$1
  local project_id=$2

  echo "Granting roles/artifactregistry.writer to service-account $workload_service_account ..."
  if ! gcloud projects add-iam-policy-binding "$project_id" \
    --member="serviceAccount:$workload_service_account@$project_id.iam.gserviceaccount.com" \
    --role="roles/artifactregistry.writer"; then
    err "Failed to grant roles/artifactregistry.writer to service-account $workload_service_account."
  fi
}

#######################################
# Grants roles/compute.serviceAgent to service-account.
# Globals:
#   None
# Arguments:
#   Workload service account
#   Project ID
#######################################
grant_compute_service_agent_role_to_service_account() {
  local workload_service_account=$1
  local project_id=$2

  echo "Granting roles/compute.serviceAgent to service-account $workload_service_account ..."
  if ! gcloud projects add-iam-policy-binding "$project_id" \
    --member="serviceAccount:$workload_service_account@$project_id.iam.gserviceaccount.com" \
    --role="roles/compute.serviceAgent"; then
    err "Failed to grant roles/compute.serviceAgent to service-account $workload_service_account."
  fi
}

#######################################
# Grants roles/run.developer to service-account.
# Globals:
#   None
# Arguments:
#   Workload service account
#   Project ID
#######################################
grant_run_developer_role_to_service_account() {
  local workload_service_account=$1
  local project_id=$2

  echo "Granting roles/run.developer to service-account $workload_service_account ..."
  if ! gcloud projects add-iam-policy-binding "$project_id" \
    --member="serviceAccount:$workload_service_account@$project_id.iam.gserviceaccount.com" \
    --role="roles/run.developer"; then
    err "Failed to grant roles/run.developer to service-account $workload_service_account."
  fi
}

#######################################
# Grants roles/run.invoker to service-account.
# Globals:
#   None
# Arguments:
#   Workload service account
#   Project ID
#######################################
grant_run_invoker_role_to_service_account() {
  local workload_service_account=$1
  local project_id=$2

  echo "Granting roles/run.invoker to service-account $workload_service_account ..."
  if ! gcloud projects add-iam-policy-binding "$project_id" \
    --member="serviceAccount:$workload_service_account@$project_id.iam.gserviceaccount.com" \
    --role="roles/run.invoker"; then
    err "Failed to grant roles/run.invoker to service-account $workload_service_account."
  fi
}

#######################################
# Grants roles/cloudscheduler.admin to service-account.
# Globals:
#   None
# Arguments:
#   Workload service account
#   Project ID
#######################################
grant_cloud_scheduler_admin_role_to_service_account() {
  local workload_service_account=$1
  local project_id=$2

  echo "Granting roles/cloudscheduler.admin to service-account $workload_service_account ..."
  if ! gcloud projects add-iam-policy-binding "$project_id" \
    --member="serviceAccount:$workload_service_account@$project_id.iam.gserviceaccount.com" \
    --role="roles/cloudscheduler.admin"; then
    err "Failed to grant roles/cloudscheduler.admin to service-account $workload_service_account."
  fi
}
