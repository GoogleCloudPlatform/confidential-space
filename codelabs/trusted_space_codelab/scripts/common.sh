...
#######################################
create_kms_keyring() {
  gcloud kms keyrings describe "${1}" --location="${2}" 2>/dev/null | grep "${1}"
  if [[ $? -eq 0 ]]; then
...
create_kms_encryption_key() {
  gcloud kms keys describe "${1}" --keyring="${2}" --location="${3}" 2>/dev/null | grep "${1}"
  if [[ $? -eq 0 ]]; then
...
#######################################
destroy_kms_key() {
  gcloud kms keys describe "${2}" --keyring="${3}" --location="${4}" | grep "ENABLED"
  if [[ $? -eq 0 ]]; then
    gcloud kms keys versions destroy "${1}" --key "${2}" --keyring "${3}" --location "${4}"
    if [[ $? -eq 0 ]]; then
      echo "Key "${2}" is deleted successfully."
    else
      err "Failed to delete a key "${2}"."
    fi
  else
    echo "Key "${2}" doesn't exist. Skipping the deletion of the key "${2}"..."
  fi
}
...
#   Location of workload identity pool
#######################################
create_workload_identity_pool() {
  gcloud iam workload-identity-pools describe "${1}" --location="${2}" 2>/dev/null | grep "${1}"
  if [[ $? -eq 0 ]]; then
...
delete_workload_identity_pool() {
  gcloud iam workload-identity-pools describe "${1}" --location="${2}" 2>/dev/null | grep "ACTIVE"
  if [[ $? -eq 0 ]]; then
